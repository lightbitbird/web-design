(function() {
    ...
    magicFocus = class magicFocus {
        ...
        show() {
            var base, base1, el
            if (!(typeof(base = ['INPUT', 'SELECT', 'TEXTAREA']).includes ===
                    'function' ?
                    base.includes((el = document.activeElement).nodeName) :
                    void 0)) {
                return
            }
            clearTimeout(this.reset)
            if (
                typeof(base1 = ['checkbox', 'radio']).includes === 'function' ?
                base1.includes(el.type) :
                void 0
            ) {
        ...
        hide() {
            var base, el
            if (!(typeof(base = ['INPUT', 'SELECT', 'TEXTAREA', 'LABEL']).includes ===
                    'function' ?
                    base.includes((el = document.activeElement).nodeName) :
                    void 0)) {
